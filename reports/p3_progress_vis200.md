...
| `p3_eval_ch02_a.yaml` | 28 | evt_000401–evt_000428 | calc×9 / concept×8 / choice×4 / term×2 / regime_trap×2 / unanswerable×2 / contrast×1 | 2 | **0** |
| `p3_eval_ch04_a.yaml` | 25 | evt_000429–evt_000453 | concept×6 / calc×5 / unanswerable×4 / term×3 / choice×3 / regime_trap×3 / contrast×1 | 4 | **0** |
| `p3_eval_ch05_a.yaml` | 22 | evt_000454–evt_000475 | calc×6 / concept×5 / term×3 / choice×3 / regime_trap×2 / unanswerable×2 / contrast×1 | 2 | **0** |
| `p3_eval_mm_a.yaml` | 28 | evt_000476–evt_000503 | calc×6 / concept×6 / term×4 / choice×4 / unanswerable×4 / contrast×2 / clarify×2 | 4 | **0** |
| `p3_eval_vis_a.yaml` | 17 | evv_000401–evv_000417 | figure_qa×8 / compare×4 / readout×3 / trend×2 | 0 | **0** |
| `p3_mm_doppler_a.yaml` | 25 | txt_001441–txt_001465 | calc×10 / regime_trap×5 / choice×5 / contrast×4 / unanswerable×1 | 1 | **0** |
...
| `p3_vis_ch03_a.yaml` | 19 | vis_000425–vis_000443 | trend×6 / readout×5 / figure_qa×4 / compare×4 | 0 | **0** |
| `p3_vis_ch05_a.yaml` | 16 | vis_000444–vis_000459 | figure_qa×7 / readout×4 / trend×3 / compare×2 | 0 | **0** |
| `p3_vis_ch07_a.yaml` | 20 | vis_000460–vis_000479 | figure_qa×9 / compare×5 / readout×4 / trend×2 | 0 | **0** |
| `p3_vis_exp1.yaml` | 15 | vis_000480–vis_000494 | readout×7 / figure_qa×7 / trend×1 | 0 | **2** |
| `p3_vis_exp2.yaml` | 15 | vis_000495–vis_000509 | readout×6 / figure_qa×6 / trend×3 | 0 | **0** |
| `p3_vis_exp3.yaml` | 14 | vis_000510–vis_000523 | readout×8 / figure_qa×6 | 0 | **5** |
| `p3_vis_exp4.yaml` | 25 | vis_000524–vis_000548 | figure_qa×13 / readout×12 | 0 | **0** |
| `p3_vis_exp5.yaml` | 24 | vis_000549–vis_000572 | figure_qa×16 / readout×8 | 0 | **2** |
| `p3_vis_exp6.yaml` | 20 | vis_000573–vis_000592 | figure_qa×17 / readout×3 | 0 | **4** |
| `p3_vis_exp7.yaml` | 8 | vis_000593–vis_000600 | readout×5 / figure_qa×3 | 0 | **0** |
...
|---|---|---|---|---|
| `calc` | 241 | 200 | +41 | 120% |
| `concept` | 276 | 250 | +26 | 110% |
| `term` | 123 | 110 | +13 | 112% |
| `choice` | 149 | 130 | +19 | 115% |
| `regime_trap` | 83 | 70 | +13 | 119% |
| `contrast` | 71 | 60 | +11 | 118% |
| `unanswerable` | 55 | 40 | +15 | 138% |
| `clarify` | 22 | 20 | +2 | 110% |
| 视觉 4 类 | 221 | 83 | 0 | 266% |
...
- `universal` 690 (56%)
- `airborne_pulse` 273 (22%)
- `automotive_mmwave` 102 (8%)
- `fmcw` 59 (5%)
- `sar` 44 (4%)
- `phased_array` 37 (3%)
- `unspecified` 22 (2%)
...
- `detection` 147 (12%)
- `doppler` 121 (10%)
- `pulse_compression` 120 (10%)
- `radar_equation` 108 (9%)
- `clutter` 107 (9%)
- `array_processing` 98 (8%)
- `resolution` 94 (8%)
- `tracking` 66 (5%)
- `imaging` 56 (5%)
...
- **universal 占比**：56%（需 ≥30%）✓
- **clarify 占比**：1.8%（需 ≤10%）✓

### 4.5 `calc` 参数来源（⚠️ 训练集取 `train` 半边、评测集取 `eval` 半边，**同参数不得跨半边**）

- 训练集 calc **215** 条：取 `train` 半边 **215** 条 ✓
- 评测集 calc **26** 条：取 `eval` 半边 **26** 条 ✓
- 不在参数池（硬写）：**0** 条
- 公式名不在参数池：**0** 条
...
⚠️ **13 条未命中**（需人工核对：可能是引用时改写、跨语料、或编造）

| 单元 | id | task | source | quote |
|---|---|---|---|---|
| p3_vis_exp1.yaml | `vis_000488` | readout | 《机载雷达系统与信息处理》第2章 图2.4.20（P | 图2.4.20 巴克码序列的速度模糊函数 |
| p3_vis_exp1.yaml | `vis_000489` | figure_qa | 《机载雷达系统与信息处理》第2章 图2.4.20（P | 图2.4.20 巴克码序列的速度模糊函数 |
| p3_vis_exp3.yaml | `vis_000512` | readout | 《机载雷达系统与信息处理》第3章 图 3.3.11（ | 图 3.3.11 不同频率下的 σ0 |
| p3_vis_exp3.yaml | `vis_000513` | figure_qa | 《机载雷达系统与信息处理》第3章 图 3.3.11（ | 图 3.3.11 不同频率下的 σ0 |
| p3_vis_exp3.yaml | `vis_000517` | readout | 《机载雷达系统与信息处理》第3章 图 3.3.12（ | 图 3.3.12 海面 σ0 与风向的关系 |
| p3_vis_exp3.yaml | `vis_000522` | readout | 《机载雷达系统与信息处理》第3章 图3.3.22(a | 不同距离单元空间自相关函数 |
| p3_vis_exp3.yaml | `vis_000523` | figure_qa | 《机载雷达系统与信息处理》第3章 图3.3.22(a | 不同距离单元空间自相关函数 |
| p3_vis_exp5.yaml | `vis_000571` | figure_qa | 《机载雷达系统与信息处理》第7章 图 7.3.7（P | 图 7.3.7 有源 ESA 通过消除 LNA 前的损耗源 |
| p3_vis_exp5.yaml | `vis_000572` | figure_qa | 《机载雷达系统与信息处理》第7章 图 7.3.7（P | 图 7.3.7 有源 ESA 通过消除 LNA 前的损耗源 |
| p3_vis_exp6.yaml | `vis_000581` | figure_qa | 《机载雷达系统与信息处理》第3章 图3.3.21（P | 海杂波与海尖峰的回波幅度序列 |
| p3_vis_exp6.yaml | `vis_000582` | figure_qa | 《机载雷达系统与信息处理》第7章 图7.5.3（PD | 均匀加权波束形成器的输出方向图 |
| p3_vis_exp6.yaml | `vis_000583` | figure_qa | 《机载雷达系统与信息处理》第5章 图5.1.8（PD | 大气衰减随频率的变化 |
| p3_vis_exp6.yaml | `vis_000584` | figure_qa | 《机载雷达系统与信息处理》第7章 图7.5.3（PD | 不加窗与 −40 dB、−60 dB 两种切比雪夫权加窗后的归一化方向图 |
